 ...
 * Import Budget
 * 
 * @author Sachin Bhimani [www.Logilite.com]
 */
public class ImportBudget extends SvrProcess
{
	...
	private boolean updateTable() throws SQLException
	{
		...
		{
			sql = new StringBuffer("DELETE I_Budget  " + "WHERE I_IsImported='Y'").append(clientCheck);
			no = DB.executeUpdate(sql.toString(), get_TrxName());
			log.fine("Delete Old Impored =" + no);
		}

		// Set IsActive, Created/Updated,Client
		sql = new StringBuffer("UPDATE I_Budget  " + "SET IsActive = COALESCE (IsActive, 'Y'),"
				...
				+ " I_ErrorMsg = ' '," + " I_IsImported = 'N' , " + " AD_Client_ID= " + Env.getAD_Client_ID(getCtx())
				+ " WHERE (I_IsImported<>'Y' OR I_IsImported IS NULL) ").append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.info("Reset=" + no);

		// Set organization
		sql = new StringBuffer("UPDATE I_Budget i	SET AD_Org_ID=").append(m_AD_Org_ID)
				.append(" WHERE I_IsImported<>'Y'").append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.fine("Set Client Organizarion=" + no);

		// Set Accounting Schema from name
		sql = new StringBuffer("UPDATE I_Budget i "
				...
				+ "WHERE C_AcctSchema_ID IS NULL AND AcctSchemaName IS NOT NULL AND I_IsImported<>'Y'").append(
				clientCheck).append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());

		// Set default accounting schema
		sql = new StringBuffer("UPDATE I_Budget i " + "SET C_AcctSchema_ID= ").append(m_C_AcctSchema_ID)
				.append(" WHERE  I_IsImported<>'Y'").append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql = new StringBuffer(
				"UPDATE I_Budget i SET GL_Budget_ID= (SELECT g.GL_Budget_ID From GL_Budget g where i.BudgetCode=g.Name and i.AD_Client_ID= g.AD_Client_ID ) "
						+ " WHERE GL_Budget_ID IS NULL and BudgetCode IS NOT NULL AND I_IsImported<>'Y'").append(
				clientCheck).append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql = new StringBuffer("UPDATE I_Budget i "
				...
				+ " WHERE (GL_Budget_ID IS NULL OR GL_Budget_ID=0)" + " AND I_IsImported<>'Y'").append(clientCheck)
				.append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
		{
		...
		// Set Asset
		sql = new StringBuffer("UPDATE I_Budget i " + "SET A_Asset_ID=(SELECT a.A_Asset_ID FROM A_Asset a"
				+ " WHERE a.Value=i.AssetValueAmt AND  i.AD_Client_ID=a.AD_Client_ID) "
				+ "WHERE A_Asset_ID IS NULL AND AssetValueAmt IS NOT NULL"
				+ " AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'").append(
				clientCheck).append(docCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		// Set BPartner
		sql = new StringBuffer("UPDATE I_Budget i " + "SET C_BPartner_ID=(SELECT bp.C_BPartner_ID FROM C_BPartner bp"
		...
		// Set Product
		sql = new StringBuffer("UPDATE I_Budget i " + "SET M_Product_ID=(SELECT MAX(p.M_Product_ID) FROM M_Product p"
				+ " WHERE (p.Value=i.ProductValue)" + " AND p.IsSummary='N' AND i.AD_Client_ID=p.AD_Client_ID) "
		...
		// Set Project
		sql = new StringBuffer("UPDATE I_Budget i " + "SET C_Project_ID=(SELECT p.C_Project_ID FROM C_Project p"
		...
		// Set TrxOrg
		sql = new StringBuffer("UPDATE I_Budget i " + "SET AD_OrgTrx_ID=(SELECT o.AD_Org_ID FROM AD_Org o"
		...
		// Set Campaign
		sql = new StringBuffer("UPDATE I_Budget i " + "SET C_Campaign_ID=(SELECT c.C_Campaign_ID FROM C_Campaign c"
	...
	private String checkBalance()
	{
		...
		StringBuffer sql = new StringBuffer(
				"SELECT SUM(COALESCE(Month_0_Amt,0)),SUM(COALESCE(Month_1_Amt,0)),SUM(COALESCE(Month_2_Amt,0)), "
						...
						+ "FROM I_Budget  " + "WHERE I_IsImported='N'").append(clientCheck).append(docCheck)
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if (month_0_Amt.compareTo(BigDecimal.ZERO) != 0 || month_1_Amt.compareTo(BigDecimal.ZERO) != 0
						|| month_2_Amt.compareTo(BigDecimal.ZERO) != 0 || month_3_Amt.compareTo(BigDecimal.ZERO) != 0
						|| month_4_Amt.compareTo(BigDecimal.ZERO) != 0 || month_5_Amt.compareTo(BigDecimal.ZERO) != 0
						|| month_6_Amt.compareTo(BigDecimal.ZERO) != 0 || month_7_Amt.compareTo(BigDecimal.ZERO) != 0
						|| month_8_Amt.compareTo(BigDecimal.ZERO) != 0 || month_9_Amt.compareTo(BigDecimal.ZERO) != 0
						|| month_10_Amt.compareTo(BigDecimal.ZERO) != 0 || month_11_Amt.compareTo(BigDecimal.ZERO) != 0)
				{
	...
	private boolean checkQty()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if ((month_0_Amt.signum() != 0 && month_0_Qty.signum() != 0 && month_0_Amt.signum() != month_0_Qty
						.signum())
						|| (month_1_Amt.signum() != 0 && month_1_Qty.signum() != 0 && month_1_Amt.signum() != month_1_Qty
								.signum())
						|| (month_2_Amt.signum() != 0 && month_2_Qty.signum() != 0 && month_2_Amt.signum() != month_2_Qty
								.signum())
						|| (month_3_Amt.signum() != 0 && month_3_Qty.signum() != 0 && month_3_Amt.signum() != month_3_Qty
								.signum())
						|| (month_4_Amt.signum() != 0 && month_4_Qty.signum() != 0 && month_4_Amt.signum() != month_4_Qty
								.signum())
						|| (month_5_Amt.signum() != 0 && month_5_Qty.signum() != 0 && month_5_Amt.signum() != month_5_Qty
								.signum())
						|| (month_6_Amt.signum() != 0 && month_6_Qty.signum() != 0 && month_6_Amt.signum() != month_6_Qty
								.signum())
						|| (month_7_Amt.signum() != 0 && month_7_Qty.signum() != 0 && month_7_Amt.signum() != month_7_Qty
								.signum())
						|| (month_8_Amt.signum() != 0 && month_8_Qty.signum() != 0 && month_8_Amt.signum() != month_8_Qty
								.signum())
						|| (month_9_Amt.signum() != 0 && month_9_Qty.signum() != 0 && month_9_Amt.signum() != month_9_Qty
								.signum())
						|| (month_10_Amt.signum() != 0 && month_10_Qty.signum() != 0 && month_10_Amt.signum() != month_10_Qty
								.signum())
						|| (month_11_Amt.signum() != 0 && month_11_Qty.signum() != 0 && month_11_Amt.signum() != month_11_Qty
								.signum()))

				{
					sql = new StringBuffer(
							"UPDATE I_Budget i "
									...
									+ " Where I_IsImported<>'Y' ").append(clientCheck).append(docCheck)
							.append(" AND I_Budget_ID=").append(i_Budget_ID);
	...
	{
		gl_Category_ID = DB.getSQLValue(get_TrxName(),
				"SELECT GL_Category_ID FROM GL_Category WHERE Name = 'Manual' AND AD_Client_ID =? ",
				Env.getAD_Client_ID(getCtx()));
		c_Currency_ID = DB.getSQLValue(get_TrxName(),
				"SELECT C_Currency_ID FROM c_acctschema WHERE c_acctschema_id = ?", m_C_AcctSchema_ID);
		c_DocType_ID = DB.getSQLValue(get_TrxName(),
				"SELECT C_DocType_ID FROM C_DocType WHERE Name = 'GL Journal' AND AD_Client_ID =?",
				Env.getAD_Client_ID(getCtx()));
		c_ConType_ID = DB.getSQLValue(get_TrxName(), "SELECT C_ConversionType_ID FROM C_ConversionType WHERE Value=? ",
				"S");
	}
	...
	private MJournalBatch createGlBatch()
	{
		...
		if (journalBatch.getGL_JournalBatch_ID() > 0)
		{
			...
			StringBuffer sql = new StringBuffer("UPDATE I_Budget " + " SET GL_JournalBatch_ID= ")
					.append(journalBatch.getGL_JournalBatch_ID() + " WHERE I_IsImported='N' ").append(clientCheck)
					.append(docCheck);
			DB.executeUpdate(sql.toString(), get_TrxName());
	...
	private void createJournal(MJournalBatch mjb)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM I_Budget WHERE I_IsImported='N' ").append(clientCheck)
				.append(docCheck);
		...
		try
		{
			...
			if (rs.next()) // first only
			{
...

				{
					...
					journal.setClientOrg(journal.getGL_JournalBatch().getAD_Client_ID(), journal.getGL_JournalBatch()
							.getAD_Org_ID());
			...
			while (rs.next())
			{
				...
				if (amt != null && amt.compareTo(BigDecimal.ZERO) != 0)
				{
					...
					MAccount acct = MAccount.get(getCtx(), rs.getInt("AD_Client_ID"), rs.getInt("AD_Org_ID"),
							rs.getInt("C_AcctSchema_ID"), rs.getInt("Account_ID"), 0, rs.getInt("M_Product_ID"),
							rs.getInt("C_BPartner_ID"), rs.getInt("AD_OrgTrx_ID"), rs.getInt("C_LocFrom_ID"),
							rs.getInt("C_LocTo_ID"), rs.getInt("C_SalesRegion_ID"), rs.getInt("C_Project_ID"),
							rs.getInt("C_Campaign_ID"), rs.getInt("C_Activity_ID"), rs.getInt("User1_ID"),
							rs.getInt("User2_ID"), 0, 0, get_TrxName());
					...
					else
					{
						...
						sql = new StringBuffer("UPDATE I_Budget  " + "SET C_ValidCombination_ID =")
								.append(acct.get_ID()).append("WHERE  I_IsImported<>'Y' and I_Budget_ID=")
								.append(rs.getInt("I_Budget_ID")).append(clientCheck).append(docCheck);
						DB.executeUpdate(sql.toString(), get_TrxName());
					}
	...
	{
		String sql = "SELECT BatchDocumentNo, I_IsImported FROM I_Budget WHERE I_Budget_ID=" + getRecord_ID();
		...
		if (message != null && message.equals("Document Ok") && finCalendar > 0)
		{
			...
			if (journalBatch != null)
			{
				...
				sql = "UPDATE I_Budget	SET I_IsImported='Y', Processed='Y', Updated=SysDate "
						+ "WHERE I_IsImported='N' AND GL_JournalBatch_ID IS NOT NULL " + clientCheck + docCheck;
				i = DB.executeUpdate(sql, get_TrxName());
				log.fine("Imported =" + i);
				if (i > 0)
